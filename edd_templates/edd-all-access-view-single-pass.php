...
if ( is_user_logged_in() ) {
	...
	if ( 'invalid' == $all_access_pass->status ) {
		...
		?>
		<h3><?php echo $download->get_name(); ?></h3>
		<p>The following details are for your <?php echo $download->get_name(); ?> purchase.</p>
		...
		<table id="edd_all_access_pass_details" class="edd_all_access_table">
					...
					<td class="row-value">
							<span class="edd-aa-expiration-date-value"><?php
								...
								if ( 'upgraded' == $all_access_pass->status ){
									...
									echo $all_access_pass->expiration_time == 'never' ? __( 'Never Expires', 'edd-all-access' ) : date( 'M d, Y, g:i a', $all_access_pass->expiration_time );
								}
								?></span>
					</td>
					<?php do_action( 'edd_all_access_pass_details_row_end', $all_access_pass ); ?>
				...
				<tr class="edd_all_access_pass_details_row">
					...
					<td class="row-value">
						...
						if( 'never' == $all_access_pass->expiration_time ){
							...
							echo sprintf( __( 'You have %i renewal payments but your current pass is set to never expire.', 'edd-all-access' ), count( $all_access_pass->renewal_payment_ids ) );
							echo '</span>';
						}else{
							...
							foreach ( $all_access_pass->renewal_payment_ids as $renewal_payment_id ) {
								...
								echo __( 'Start Date', 'edd-all-access' ) . ': ' . date( 'M d, Y', $all_access_pass->start_time + ( $duration_time * $loop_num ) );
								echo '</span>';
								echo '<span class="edd-aa-upcoming-expiration-date">';
								echo __( 'Expiration Date', 'edd-all-access' ) . ': ' . date( 'M d, Y', $all_access_pass->start_time + ( $duration_time * ( $loop_num + 1 ) ) );
								echo '</span>';
								echo '</span>';
			...
			<tr class="edd_all_access_pass_details_row edd_all_access_status_row">
				<?php do_action( 'edd_all_access_pass_details_row_start', $all_access_pass ); ?>
				<td class="row-label">
